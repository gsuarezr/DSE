...
class zero_temp_bath(heom.CorrelationFitter):
    def __init__(self,Q,time,lam,gamma,w0,N=2,lower=None,upper=None,sigma=None,guesses=None):
        Gamma=gamma/2
        Omega=np.sqrt(w0**2 - Gamma**2)
        C=np.array([self._matsubara_zero_integrand(i,lam,gamma,w0) for i in time])
        self.correlation_function=C+self.C0(time,lam,gamma,w0)
        super().__init__(Q,0,time,C)
        self.fbath,self.finfo =self.get_fit(Ni=1,Nr=N,lower=lower,upper=upper,sigma=sigma,guesses=guesses)
        ck=(lam**2)/(2*Omega)
        vk=(-Gamma+1j*Omega)
        C0_contrib=[BathExponent(type="R",dim=None,Q=Q,ck=ck/2,vk=-vk),BathExponent(type="R",dim=None,Q=Q,ck=ck/2,vk=-np.conjugate(vk))]
        self.fbath.exponents.extend(C0_contrib)
        self.exponents=self.fbath.exponents
        self.bath=self.fbath
    def _matsubara_zero_integrand(self,t, coup_strength, bath_broad, bath_freq):
        """
