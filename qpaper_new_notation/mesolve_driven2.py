...
# To handle zero-temperature underdamped bath with HEOM, fit its correlation function
cfit,fit_info =bath.approx_by_cf_fit(fit_times,Ni_max=1,Nr_max=3)
print(fit_info['summary'])
# Convert to a HEOM bath
heombath=BosonicBath.from_environment(cfit,qt.sigmax())

# Do simulation
HEOM_corr_fit = HEOMSolver(qt.QobjEvo(H), heombath, max_depth=max_depth)
...
# To handle zero-temperature underdamped bath with HEOM, fit its correlation function
cfit,fit_info =bath.approx_by_cf_fit(fit_times,Ni_max=1,Nr_max=2,target_rsme=None)
print(fit_info["summary"])
# Convert to a HEOM bath
heombath=BosonicBath.from_environment(cfit,qt.sigmax())
...
a_ops = [[qt.sigmax(), lambda x: bath.power_spectrum(x)]]
brme_result = qt.brmesolve(H, psi0, tlist, a_ops=a_ops, e_ops=e_ops,options={'nsteps': 15000, 'rtol': 1e-12, 'atol': 1e-12},sec_cutoff=-1)
