...
from environment import UnderDampedEnvironment
...
heombath = cfit.to_bath(Q=qt.sigmax())
...
cfit, fit_info = bath.approx_by_cf_fit(
    fit_times, Ni_max=1, Nr_max=2, target_rsme=None)
print(fit_info["summary"])
# Convert to a HEOM bath
heombath = cfit.to_bath(qt.sigmax())
...
a_ops = [[qt.sigmax(), lambda w:cfit.power_spectrum(w).item()]]
